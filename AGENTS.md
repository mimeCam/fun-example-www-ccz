...
- `scripts/audit-spacing-collapse.ts` — local-only static scanner for portal-`null` + sibling-`mt-sys-N` collisions. Runs report-only; not in CI. Tune the regex when the next finding earns it (Mike #2 §6.5).
