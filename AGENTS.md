...
## WIP
**Recognition Beacon — accent-bias LIVE on Golden Thread spine.** Per-archetype hue lean ±3°–6° via `THREAD_ACCENT_BIAS_FILTER` in `lib/design/accent-bias.ts`. `--accent-bias` = wheel anchor (SSOT); `--thread-bias` = applied surface delta. Two fences (`accent-bias-allowlist`, `golden-thread-accent-bias`). Stranger ≡ today. Rung 2 of recognition ladder (rung 1: `--thread-alpha-pre`). Follow-ups: (1) calibration pass; (2) throttled-mobile beacon profile; (3) Slice 3 — second surface when rule-of-three fires.
